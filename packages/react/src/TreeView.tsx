...
export type TreeViewDensity = FlowDensity;
...
interface NormalizedTreeViewNode extends TreeViewNode {
  key: string;
  level: number;
  expandable: boolean;
  expanded: boolean;
...
const validStates = new Set<TreeViewState>(["default", "hover", "focus", "expanded", "selected", "disabled"]);

function nodeKey(node: TreeViewNode) {
  return String(node?.key ?? node?.id);
}

function normalizeNodes(nodes: TreeViewNode[]): NormalizedTreeViewNode[] {
...
function visibleKeys(nodes: NormalizedTreeViewNode[], expandedKeys: string[]) {
  const hiddenByLevel: number[] = [];
  return nodes.filter((node) => {
    while (hiddenByLevel.length && (hiddenByLevel[hiddenByLevel.length - 1] ?? 0) >= node.level) hiddenByLevel.pop();
...
export const TreeView = forwardRef<HTMLUListElement, TreeViewProps>(function TreeView({
  ...
  const focusKey = (key: string | undefined) => requestAnimationFrame(() => {
    if (key) controlRefs.current.get(key)?.focus();
  });
  const commitSelected = (node: NormalizedTreeViewNode | undefined, event: MouseEvent<HTMLButtonElement>) => {
    if (!node || node.disabled) return;
    if (!isSelectedKeyControlled) setInternalSelected(node.key);
    onSelect?.(node.key, event);
    focusKey(node.key);
  };
  const commitExpanded = (node: NormalizedTreeViewNode | undefined, nextExpanded: boolean, event: TreeViewExpandedChangeEvent) => {
  ...
  const move = (node: NormalizedTreeViewNode, direction: number) => {
    const enabled = visible
      .map((key) => normalizedNodes.find((candidate) => candidate.key === key))
      .filter((candidate): candidate is NormalizedTreeViewNode => Boolean(candidate && !candidate.disabled));
  ...
  const edge = (position: "first" | "last") => {
    const enabled = visible
      .map((key) => normalizedNodes.find((candidate) => candidate.key === key))
      .filter((candidate): candidate is NormalizedTreeViewNode => Boolean(candidate && !candidate.disabled));
