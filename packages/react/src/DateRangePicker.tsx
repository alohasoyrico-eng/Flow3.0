...
type ResolvedDateRange = { from: string; to: string };
...
function formatDateLabel(value: string | undefined, locale: DateRangePickerLocale): string {
...
function formatDateLongLabel(value: string | undefined, locale: DateRangePickerLocale): string {
  const date = parseDate(value);
  if (!date) return "";
  return new Intl.DateTimeFormat(locale, { weekday: "long", day: "numeric", month: "long", year: "numeric" }).format(date);
}

function formatMonthLabel(date: Date, locale: DateRangePickerLocale): string {
...
function resolveDateRangePickerState({
  ...
  error?: string;
  invalid?: boolean;
  state?: DateRangePickerState | undefined;
  from?: string;
  to?: string;
...
function rangeLabel({ from, to, placeholder, locale }: ResolvedDateRange & { placeholder: string; locale: DateRangePickerLocale }): string {
...
export const DateRangePicker = forwardRef<HTMLButtonElement, DateRangePickerProps>(function DateRangePicker({
  ...
  const [internalRange, setInternalRange] = useState<ResolvedDateRange>({ from: initialFrom, to: initialTo });
  ...
  const selectDate = (nextValue: string, event: DateRangePickerValueChangeEvent): void => {
  ...
  const applyPreset = (preset: DateRangePickerPreset, event: MouseEvent<HTMLButtonElement>): void => {
    const end = new Date();
  ...
  return React.createElement(
    ...
    React.createElement("input", {
      ...
      "aria-hidden": "true",
      onChange: (event: ChangeEvent<HTMLInputElement>) => commitRange({ from: event.target.value, to: range.to }, false, event),
    }),
    React.createElement("input", {
      type: "date",
      className: "date-picker__input date-range-picker__input",
      ...
      "aria-hidden": "true",
      onChange: (event: ChangeEvent<HTMLInputElement>) => commitRange({ from: range.from, to: event.target.value }, false, event),
    }),
    React.createElement(
      "div",
      {
