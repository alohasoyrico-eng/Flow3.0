...
const validVariants = new Set<ChartPanelVariant>(["sparkline", "bars", "line", "area", "donut", "pareto", "bullet", "comparison", "compact"]);
...
function renderBullet(values: number[], labels: string[]) {
  const points = normalizePoints(values, labels);
  const max = Math.max(...points.map((point) => point.value), 1);
...
function renderPareto(values: number[], labels: string[]) {
  ...
  return React.createElement(
    ...
    sorted.map((item, index) => React.createElement("rect", {
      ...
      style: { "--comp-chart-panel-stagger-delay": chartStaggerDelay(index) } as ChartPanelStyle,
    })),
...
export const ChartPanel = forwardRef<HTMLElement, ChartPanelProps>(function ChartPanel({
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement("div", { className: "chart-panel__plot", role: "list" }, renderPlot(normalizeVariant(chartPrimitive.type as ChartPanelVariant), resolvedValues, resolvedLabels, resolvedSeries, resolvedComparisons, resolvedSegments)),
