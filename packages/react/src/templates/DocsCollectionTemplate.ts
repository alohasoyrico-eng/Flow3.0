...
import { Toolbar, type ToolbarProps } from "../patterns/Toolbar.js";
...
export const DocsCollectionTemplate = forwardRef<HTMLElement, DocsCollectionTemplateProps>(function DocsCollectionTemplate({
  title,
  description,
  metadata = [],
  search,
  ...
  return React.createElement(
    ...
    React.createElement(SectionHeader, { title, ...(description ? { description } : {}), headingLevel: 1, density, "data-flow-slot": "docs-collection.header" }),
    metadata.length ? React.createElement(ArtifactMetadataBar, { items: metadata, density, "data-flow-slot": "docs-collection.metadata" }) : null,
    React.createElement(Surface, { surfaceRole: "section", density, tone: "default", elevation: "none", "data-flow-slot": "docs-collection.controls" },
      search ? React.createElement(Search, { ...search, density: search.density ?? density, "data-flow-slot": "docs-collection.search" }) : null,
      toolbar ? React.createElement(Toolbar, { ...toolbar, density: toolbar.density ?? density, "data-flow-slot": "docs-collection.toolbar" }) : null,
    ),
    React.createElement(DocumentationSection, { density, state: children ? "default" : "empty", "data-flow-slot": "docs-collection.grid" }, children),
  );
}) as DocsCollectionTemplateComponent;
