...
type EmailTemplateDefaults = Partial<Pick<
  EmailTemplateLayoutProps,
  "title" | "preheader" | "eyebrow" | "headline" | "body" | "rows" | "metrics" | "alertsTitle" | "alerts" | "action" | "code" | "codeHelper" | "steps" | "note" | "tone"
>>;
...
type ButtonLinkProps = {
  action: EmailTemplateAction | undefined;
};

type KeyValueRowsProps = {
  rows: EmailTemplateRow[] | undefined;
};

type MetricCellProps = {
  metric: EmailTemplateMetric & { offset?: boolean };
};

type MetricsGridProps = {
  metrics: EmailTemplateMetric[] | undefined;
};

type AlertListProps = {
  title?: string | undefined;
  items: Array<EmailTemplateAlert | string> | undefined;
};
...
type StepListProps = {
  steps: Array<EmailTemplateStep | string> | undefined;
};

const typedEmailTokenValues = emailTokenValues as Readonly<Record<string, string>>;
type EmailTokenName = keyof typeof typedEmailTokenValues;

const token = (name: EmailTokenName): string => typedEmailTokenValues[name] ?? "";
...
function toneColor(tone: EmailTemplateLayoutTone): string {
...
function resolveVariant(variant: EmailTemplateLayoutVariant | undefined): EmailTemplateLayoutVariant {
  return variant ?? "base";
}
...
function Spacer({ size }: SpacerProps): ReactNode {
  return React.createElement("div", {
    style: { height: size, lineHeight: size, fontSize: emailHiddenSize },
  }, "\u00a0");
}

function ButtonLink({ action }: ButtonLinkProps): ReactNode {
...
function KeyValueRows({ rows }: KeyValueRowsProps): ReactNode {
...
function MetricCell({ metric }: MetricCellProps): ReactNode {
...
function AlertList({ title, items }: AlertListProps): ReactNode {
...
function CodeBlock({ code, helper }: CodeBlockProps): ReactNode {
  if (!code) return null;
...
function StepList({ steps }: StepListProps): ReactNode {
...
function variantDefaults(variant: EmailTemplateLayoutVariant): EmailTemplateDefaults {
...
export const EmailTemplateLayout = forwardRef<HTMLHtmlElement, EmailTemplateLayoutProps>(function EmailTemplateLayout({
  variant,
  density = "md",
  state = "default",
  lang = "es",
  ...
  } as CSSProperties & { msoHide: string };
...
}) as EmailTemplateLayoutComponent;
