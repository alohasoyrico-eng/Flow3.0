...
const validStates = new Set<PreferenceManagementState>(["idle", "dirty", "saving", "saved", "invalid", "danger-confirming", "permission-blocked", "disabled"]);
...
function normalizeState({
  ...
  state?: PreferenceManagementState | undefined;
  disabled: boolean;
  permissionBlocked: boolean;
  saving: boolean;
  dirty: boolean;
  dangerZone?: PreferenceManagementDangerZone | undefined;
  sections: PreferenceManagementSection[];
...
function surfaceStateFor(state: PreferenceManagementState): SurfaceState {
...
function badgeToneFor(state: PreferenceManagementState): NonNullable<BadgeProps["tone"]> {
...
function badgeStateFor(state: PreferenceManagementState): NonNullable<BadgeProps["state"]> {
  return state === "disabled" || state === "permission-blocked" ? "disabled" : "default";
}

export const PreferenceManagement = forwardRef<HTMLDivElement, PreferenceManagementProps>(function PreferenceManagement({
  ...
  const normalizedSections = normalizeArray(sections).filter((section): section is PreferenceManagementSection => Boolean(section?.title));
  ...
  return React.createElement(
    ...
    settings
      ? React.createElement(Settings, {
        ...
        onControlChange: (key: string, value: string | boolean, meta: SettingsControlChangeMeta, event: SettingsControlChangeEvent) => {
        ...
        onReset: (event: MouseEvent<HTMLButtonElement>) => {
      ...
      return React.createElement(FormSection, {
        ...
        onFieldValueChange: (fieldKey, value, meta, event) => {
          ...
          onSectionFieldValueChange?.(
            ...
            fieldKey,
            value,
            meta as InputValueMeta | TextAreaChangeMeta,
            event as ChangeEvent<HTMLInputElement> | ChangeEvent<HTMLTextAreaElement>,
          );
        },
        onAction: (actionKey: string, event: MouseEvent<HTMLButtonElement>) => {
    ...
    dangerZone?.label
      ? React.createElement(ConfirmationDialog, {
        ...
        onRecoveryAction: (key: string, event: MouseEvent<HTMLButtonElement>) => {
...
}) as PreferenceManagementComponent;
