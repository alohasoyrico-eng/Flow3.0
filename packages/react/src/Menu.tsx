...
type SetOpenOptions = { restoreFocus?: boolean; focusFirst?: boolean; event?: MenuOpenChangeEvent | undefined };

const validVariants = new Set<MenuVariant>(["actions", "grouped", "selection", "danger", "icon-trigger", "avatar-trigger"]);
const validStates = new Set<MenuState>(["default", "closed", "open", "focus", "disabled"]);
const validItemTones = new Set<MenuItemTone>(["danger"]);

function slug(value: string | undefined) {
  return String(value ?? "menu").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
}

function enabledItems(panel: HTMLDivElement | null): HTMLButtonElement[] {
  return [...(panel?.querySelectorAll?.<HTMLButtonElement>('[role="menuitem"]:not(:disabled)') ?? [])];
}

function hasStableItemKey(item: MenuItem | MenuSeparator | "divider" | undefined): item is MenuItem {
  if (!item || item === "divider" || "separator" in item) return false;
  return item?.key !== undefined && item?.key !== null && item?.key !== "";
}

function isSeparator(item: MenuItem | MenuSeparator | "divider"): item is MenuSeparator | "divider" {
...
export const Menu = forwardRef<HTMLSpanElement, MenuProps>(function Menu({
  ...
  const moveItem = (event: KeyboardEvent<HTMLDivElement>, direction: number) => {
    ...
    const index = Math.max(0, enabled.indexOf(event.target as HTMLButtonElement));
  ...
  const triggerProps = {
    ...
    onClick: (event: MouseEvent<HTMLButtonElement>) => setOpen(!isOpen, { focusFirst: !isOpen, event }),
  ...
  return React.createElement(
    ...
    hasTrigger && resolvedVariant === "icon-trigger"
      ? React.createElement(IconButton, { ...triggerProps, label: triggerLabel, icon: "more_horiz", variant: "ghost", ...(resolvedDensity ? { density: resolvedDensity as FlowDensity } : {}) })
      : hasTrigger && resolvedVariant === "avatar-trigger"
        ? React.createElement("button", { ...triggerProps, type: "button", className: "menu__trigger menu__trigger--avatar", "aria-label": triggerLabel }, React.createElement(Avatar, { name: avatarName, status: avatarStatus, ...(resolvedDensity ? { density: resolvedDensity as FlowDensity } : {}) }))
        : hasTrigger ? React.createElement(Button, { ...triggerProps, label: triggerLabel, variant: "secondary", ...(resolvedDensity ? { density: resolvedDensity as FlowDensity } : {}), trailingIcon: isOpen ? "expand_less" : "expand_more" }) : null,
    React.createElement(
      "div",
      {
        ref: panelRef,
      ...
      resolvedItems.map((item, index) => {
        if (isSeparator(item)) return React.createElement("span", { key: `separator-${index}`, className: "menu__separator", role: "separator" });
        const key = item.key;
