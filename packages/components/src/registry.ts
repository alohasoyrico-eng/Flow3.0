export type ComponentDemoPropsInput = Record<string, any>;
export type ComponentDemoPropsOutput = Record<string, any>;
type ComponentDemoItem = Record<string, any>;

function demoSlug(value: unknown): string {
  return String(value)
    .toLowerCase()
    .replace(/&/g, "and")
    .replace(/[^a-z0-9]+/g, "-")
...
export function componentDemoProps(
  component: string,
  demo: ComponentDemoPropsInput = {},
): ComponentDemoPropsOutput {
  ...
  if (component === "list") {
    ...
    const rows = listItems.map((item: ComponentDemoItem, index: number) => {
  ...
  if (component === "tabs") {
    ...
    const items = (demo.items ?? fallbackItems).map((item: ComponentDemoItem, index: number) => ({
    ...
    return { label: demo.ariaLabel ?? "Tabs demo", items, selectedKey: demo.selectedKey ?? items.find((item: ComponentDemoItem) => item.selected)?.key ?? items[0]?.key, variant: demo.variant ?? "default" };
  }
  ...
  if (component === "segmented-control") {
    ...
    return {
      ...
      items,
      selectedKey: demo.selectedKey ?? items.find((item: ComponentDemoItem) => item.selected)?.key ?? items.find((item: ComponentDemoItem) => item.selected)?.value ?? items[0]?.key ?? items[0]?.value,
      density: demo.density,
  ...
  if (component === "drawer") {
    ...
    return {
      ...
      fields: hasFields ? demo.fields.map((field: string | ComponentDemoItem) => typeof field === "string" ? { label: field, value: field === "Spending limit" ? "$2,400" : "Enabled" } : field) : [],
  ...
  if (component === "table") {
    ...
    return {
      ...
      columns: demo.columns ?? [
        ...
        { key: "spend", label: "Spend", mono: true, align: "right", sortable: tableSortable, sortValue: (row: ComponentDemoPropsOutput) => row.spendValue ?? 0, priority: "tertiary" },
      ],
      rows: demo.rows ?? [
