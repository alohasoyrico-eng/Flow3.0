...
const supportedAnimationStates = new Set<string>(["idle", "playing", "paused", "complete", "reduced-motion", "disabled"]);
const supportedRenderers = new Set<string>(["svg", "canvas", "html"]);
...
type AnimationRuntime = {
  loadAnimation: (options: Record<string, unknown>) => AnimationHandle;
};

export type AnimationAssetOptions = {
  label?: string;
  source?: string;
...
function normalizeAnimationState(state: string, reducedMotion: boolean): string {
  if (state === "disabled") return "disabled";
  if (reducedMotion || state === "reduced-motion") return "reduced-motion";
  return supportedAnimationStates.has(state) ? state : "playing";
}

function normalizeRenderer(renderer: string): string {
  return supportedRenderers.has(renderer) ? renderer : "svg";
}

function defaultAnimationRuntime(): AnimationRuntime | undefined {
  return (globalThis as typeof globalThis & { lottie?: AnimationRuntime }).lottie;
}

export function resolveAnimationRuntime(runtime: AnimationRuntime | null | undefined = defaultAnimationRuntime()): AnimationRuntime | null {
...
export function createAnimationAsset({
...
}: AnimationAssetOptions = {}): HTMLSpanElement & { __animationAsset?: AnimationHandle } {
  ...
  const root = document.createElement("span") as HTMLSpanElement & { __animationAsset?: AnimationHandle };
