...
function chartTooltipMarkerClass(index, mode = "series") {
    const size = mode === "ramp" ? chartRamp.length : chartPalette.length;
    return `chart-panel__echarts-tooltip-swatch--${mode}-${(index % size) + 1}`;
}
function chartTooltipIcon(type, label, index = 0) {
    const normalizedLabel = String(label ?? "").toLowerCase();
    if (normalizedLabel.includes("ev") || normalizedLabel.includes("energy") || normalizedLabel.includes("charge"))
        return "bolt";
    if (normalizedLabel.includes("bike"))
        return "pedal_bike";
    if (normalizedLabel.includes("fuel") || normalizedLabel.includes("diesel"))
        return "local_gas_station";
    if (normalizedLabel.includes("route") || normalizedLabel.includes("sla"))
        return "route";
    if (normalizedLabel.includes("delay") || normalizedLabel.includes("risk") || normalizedLabel.includes("alert"))
        return "warning";
    if (normalizedLabel.includes("card") || normalizedLabel.includes("wallet") || normalizedLabel.includes("payment"))
        return "payments";
    if (type === "funnel")
        return ["filter_alt", "sell", "assignment_turned_in", "local_shipping", "task_alt"][index % 5] ?? "filter_alt";
    if (type === "treemap")
        return "domain";
...
function normalizePositiveValues(values = []) {
    return values.map((item) => Math.max(0, toFiniteNumber(item)));
}
function normalizeSignedValues(values = []) {
    return values.map((item) => toFiniteNumber(item));
}
function normalizePointValues(values = []) {
    return values
        .map((item) => Array.isArray(item) ? [toFiniteNumber(item[0]), toFiniteNumber(item[1])] : null)
        .filter((item) => Boolean(item));
}
function normalizeSeriesPoints(series) {
    if (series.data?.length) {
        return series.data.map((item) => [toFiniteNumber(item.value), toFiniteNumber(item.y)]);
    }
...
function normalizeSeriesBoxValues(series) {
    if (series.data?.length)
        return normalizeBoxValues(series.data.map((item) => item.values));
...
function normalizeDataItems(series = [], values = [], labels = []) {
...
function seriesColor(index, mode, count) {
    if (mode === "categorical" || count > 3)
        return chartPalette[index % chartPalette.length] ?? chartPalette[0] ?? "var(--component-chart-series-primary)";
    if (index === 1)
        return "var(--component-chart-threshold-warning)";
    if (index === 2)
        return "var(--component-chart-series-tertiary)";
    return chartPalette[0] ?? "var(--component-chart-series-primary)";
}
function chartTooltipDatum({ value, name, type, index, markerIndex = index, markerMode = "series", }) {
    const datum = {
        value,
...
function baseSeriesRows(series, labels, values = []) {
    if (series.length) {
...
export function createChartsPrimitive({ type = "sparkline", label = "", value = "", caption = "", values = [], labels = [], series = [], comparisons = [], segments = [], matrix, thresholds = [], target, min, max, totals = [], legend = false, stack = false, horizontal = false, showValues = false, palette = "auto", indicators = [], } = {}) {
    void thresholds;
    ...
    if (["sparkline", "compact", "line", "area"].includes(resolvedType)) {
                ...
                data: segmentRows.map((item, index) => {
                    ...
                    return {
                        ...
                        emphasis: {
                            scale: true,
                            scaleSize: 4,
                            itemStyle: { color, borderColor: "var(--component-color-focus)", borderWidth: 2 },
                        },
                    };
                }),
            }];
        ...
        signed.forEach((item, index) => {
            ...
            if (isTotal) {
                ...
                bars.push({
                    value: absolute,
                    tooltipIcon: chartTooltipIcon(resolvedType, lineSeries[0]?.label || label, index),
                    tooltipMarkerClass: "chart-panel__echarts-tooltip-swatch--series-5",
                    itemStyle: { color: "var(--component-chart-series-mixed)", borderRadius: 6 },
                    emphasis: { itemStyle: { color: "var(--component-chart-series-mixed)", borderColor: "var(--component-color-focus)", borderWidth: 2 } },
                });
                running = absolute;
                return;
            }
            ...
            bars.push({
                ...
                emphasis: {
                    itemStyle: {
                        color: item >= 0 ? "var(--component-chart-series-secondary)" : "var(--component-chart-threshold-danger)",
                        borderColor: "var(--component-color-focus)",
                        borderWidth: 2,
                    },
                },
            });
        ...
        echartsOption.series = [{
                ...
                data: segmentRows.map((item, index) => {
                    ...
                    const color = chartRamp[rampIndex];
                    return {
                        name: itemLabel,
                        value: item.value,
                        tooltipIcon: chartTooltipIcon(resolvedType, itemLabel, index),
