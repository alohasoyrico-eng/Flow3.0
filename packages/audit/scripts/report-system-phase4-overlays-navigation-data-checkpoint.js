...
const jsonOutput = path.join(outputDir, "system-phase4-overlays-navigation-data-checkpoint.json");
const markdownOutput = path.join(outputDir, "system-phase4-overlays-navigation-data-checkpoint.md");

const overlaysNavigationDataIds = [
  "tabs",
  "dialog",
  "drawer",
  "menu",
...
function createReport() {
  ...
  const components = overlaysNavigationDataIds.map((id) => {
  ...
  const inventory = {
    overlaysNavigationDataComponents: overlaysNavigationDataIds.length,
    passingOverlaysNavigationDataComponents: components.filter((component) => component.status === "pass").length,
    ...
    overlaysNavigationDataDebt: issues.length,
  };

  return {
    status: issues.length ? "fail" : "pass",
    audit: "system phase 4 overlays/navigation/data checkpoint",
    principle: "Overlay, navigation, and data/display components must pass TypeScript source/runtime/declaration checks plus React primary, props, controlled state, accessibility, interaction, visual cascade, CSS contract, and style governance before domain and complex components build on them.",
    scope: "Original plan iteration 22: component fixes batch 2, overlays/navigation/data.",
    inventory,
...
function toMarkdown(report) {
  ...
  return [
    ...
    "## Inventory",
    "",
    `- Overlay/navigation/data components: ${report.inventory.passingOverlaysNavigationDataComponents}/${report.inventory.overlaysNavigationDataComponents}`,
    `- Gate reports: ${report.inventory.passingGateReports}/${report.inventory.gateReports}`,
    `- TSX sources: ${report.inventory.tsxSources}/${report.inventory.overlaysNavigationDataComponents}`,
    `- Runtime files: ${report.inventory.runtimeFiles}/${report.inventory.overlaysNavigationDataComponents}`,
    `- Declaration files: ${report.inventory.declarationFiles}/${report.inventory.overlaysNavigationDataComponents}`,
    `- Component gate edges: ${report.inventory.passingComponentGateEdges}/${report.inventory.componentGateEdges}`,
    `- Overlay/navigation/data debt: ${report.inventory.overlaysNavigationDataDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  if (checkMode) {
    ...
    if (currentJson !== nextJson || currentMarkdown !== nextMarkdown) {
      console.error("Phase 4 overlays/navigation/data checkpoint is stale. Run: node packages/audit/scripts/report-system-phase4-overlays-navigation-data-checkpoint.js");
      process.exit(1);
    }
  } else {
    writeReport(report);
  ...
  console.log(JSON.stringify({
    status: report.status,
    components: `${report.inventory.passingOverlaysNavigationDataComponents}/${report.inventory.overlaysNavigationDataComponents}`,
    ...
    debt: report.inventory.overlaysNavigationDataDebt,
    json: rel(jsonOutput),
