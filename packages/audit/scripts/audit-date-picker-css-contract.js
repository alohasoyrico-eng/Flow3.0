...
function checkDatePickerCssContract({ text, blocks, packageCssFile, selectorKey, root }) {
  ...
  for (const [snippet, message] of [
    ...
    ["mode = \"single\"", "DatePicker must own mode='single' and mode='range' instead of requiring a separate range calendar source of truth."],
    ["const isRange = mode === \"range\"", "DatePicker must derive range behavior from its mode axis."],
    ["formatRangeLabel", "DatePicker must render a readable range label when mode='range'."],
    ["commitRange", "DatePicker must own range commits instead of delegating them to DateRangePicker."],
    ["data-mode\": mode", "DatePicker must expose mode in DOM for visual/range contract governance."],
  ...
  for (const [snippet, message] of [
    ...
    ['mode: "range"', "DatePicker runtime 1:1 demo must cover mode='range' as the canonical range calendar."],
    ["presetItems:", "DatePicker runtime 1:1 demo must cover range presets from the canonical DatePicker API."],
  ]) {
