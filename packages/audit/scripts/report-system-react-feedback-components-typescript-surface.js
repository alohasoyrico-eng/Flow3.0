...
const jsonOutput = path.join(outputDir, "system-react-feedback-components-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-feedback-components-typescript-surface.md");
const checkMode = process.argv.includes("--check");
const reactSourceDir = "packages/react/src";
const components = ["EmptyState", "ErrorPanel", "InlineValidation", "Toast"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-feedback-components-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React feedback components TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React feedback component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React feedback components TypeScript surface debt: ${report.inventory.reactFeedbackComponentsTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "feedback-components-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited feedback React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more feedback React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-feedback-components-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactFeedbackComponentsTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactFeedbackComponentsTypescriptSurfaceDebt: report.inventory.reactFeedbackComponentsTypescriptSurfaceDebt,
    outputs: [
