...
const jsonOutput = path.join(outputDir, "system-react-input-localization-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-input-localization-typescript-surface.md");
...
const components = ["CodeInput", "PhoneInput", "CountrySelector", "InputAmount"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-input-localization-ts-audit-"));
...
function renderMarkdown(report) {
  ...
  return [
    ...
    `- React input/localization TypeScript surface debt: ${report.inventory.reactInputLocalizationTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "input-localization-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited input/localization React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more input/localization React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-input-localization-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactInputLocalizationTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactInputLocalizationTypescriptSurfaceDebt: report.inventory.reactInputLocalizationTypescriptSurfaceDebt,
    outputs: [
