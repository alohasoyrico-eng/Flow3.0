...
const jsonOutput = path.join(outputDir, "system-react-form-controls-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-form-controls-typescript-surface.md");
...
const components = ["IconButton", "Checkbox", "RadioButton", "Switch", "TextArea"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-form-controls-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React form controls TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React form control component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React form control component TypeScript surface debt: ${report.inventory.reactFormControlComponentTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "form-controls-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited form control React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more form control React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-form-controls-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactFormControlComponentTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactFormControlComponentTypescriptSurfaceDebt: report.inventory.reactFormControlComponentTypescriptSurfaceDebt,
    outputs: [
