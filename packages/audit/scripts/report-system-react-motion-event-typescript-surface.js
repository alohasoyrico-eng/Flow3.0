...
const jsonOutput = path.join(outputDir, "system-react-motion-event-typescript-surface.json");
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-motion-event-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React motion/event TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React motion and event-status component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React motion/event TypeScript surface debt: ${report.inventory.reactMotionEventTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "motion-event-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited motion/event React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more motion/event React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-motion-event-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactMotionEventTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactMotionEventTypescriptSurfaceDebt: report.inventory.reactMotionEventTypescriptSurfaceDebt,
    outputs: [
