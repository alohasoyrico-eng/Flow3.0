...
const jsonOutput = path.join(outputDir, "system-react-chat-components-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-chat-components-typescript-surface.md");
...
const components = ["ChatComposer", "ChatMessage", "ChatThread"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-chat-components-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React chat components TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React chat component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React chat components TypeScript surface debt: ${report.inventory.reactChatComponentsTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "chat-components-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited chat React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more chat React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-chat-components-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactChatComponentsTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactChatComponentsTypescriptSurfaceDebt: report.inventory.reactChatComponentsTypescriptSurfaceDebt,
    outputs: [
