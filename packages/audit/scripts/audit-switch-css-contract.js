...
function checkSwitchCssContract({ text, blocks, packageCssFile, selectorKey, root }) {
  const sourceFile = path.join(root || process.cwd(), "packages/react/src/Switch.js");
  ...
  for (const [block, snippets, message] of [
    ...
    [thumbBlock, ["background: var(--comp-switch-current-thumb-bg)", "block-size: var(--comp-switch-current-thumb-size)", "inline-size: var(--comp-switch-current-thumb-size)"], "Switch thumb must consume current geometry aliases."],
