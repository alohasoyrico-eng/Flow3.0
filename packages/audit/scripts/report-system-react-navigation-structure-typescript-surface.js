...
const jsonOutput = path.join(outputDir, "system-react-navigation-structure-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-navigation-structure-typescript-surface.md");
const checkMode = process.argv.includes("--check");
const reactSourceDir = "packages/react/src";
const components = ["Accordion", "TreeView", "List", "Stepper"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-navigation-structure-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React navigation structure TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React navigation and list-structure component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React navigation structure TypeScript surface debt: ${report.inventory.reactNavigationStructureTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "navigation-structure-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited navigation/list React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more navigation/list React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-navigation-structure-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactNavigationStructureTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactNavigationStructureTypescriptSurfaceDebt: report.inventory.reactNavigationStructureTypescriptSurfaceDebt,
    outputs: [
