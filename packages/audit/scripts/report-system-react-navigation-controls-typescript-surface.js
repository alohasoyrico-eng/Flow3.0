...
const jsonOutput = path.join(outputDir, "system-react-navigation-controls-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-navigation-controls-typescript-surface.md");
...
const components = ["Breadcrumbs", "Pagination", "SegmentedControl", "Slider"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-navigation-controls-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React navigation controls TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React navigation and data control TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React navigation control TypeScript surface debt: ${report.inventory.reactNavigationControlTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "navigation-controls-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited navigation React control must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more navigation React control runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-navigation-controls-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactNavigationControlTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactNavigationControlTypescriptSurfaceDebt: report.inventory.reactNavigationControlTypescriptSurfaceDebt,
    outputs: [
