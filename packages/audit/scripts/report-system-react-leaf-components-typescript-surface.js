...
const jsonOutput = path.join(outputDir, "system-react-leaf-components-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-leaf-components-typescript-surface.md");
...
const components = ["Avatar", "Badge", "ProgressIndicator", "Spinner", "Skeleton", "Tag", "Chip"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-leaf-components-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React leaf components TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React leaf component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React leaf component TypeScript surface debt: ${report.inventory.reactLeafComponentTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "leaf-components-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited leaf React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more leaf React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-leaf-components-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactLeafComponentTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactLeafComponentTypescriptSurfaceDebt: report.inventory.reactLeafComponentTypescriptSurfaceDebt,
    outputs: [
