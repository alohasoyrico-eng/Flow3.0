...
const jsonOutput = path.join(outputDir, "system-react-overlay-components-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-overlay-components-typescript-surface.md");
const checkMode = process.argv.includes("--check");
const reactSourceDir = "packages/react/src";
const components = ["Tabs", "Dialog", "Drawer", "Menu", "Popover"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-overlay-components-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React overlay components TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React overlay and navigation component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React overlay component TypeScript surface debt: ${report.inventory.reactOverlayComponentTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "overlay-components-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited base overlay React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more base overlay React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-overlay-components-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactOverlayComponentTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactOverlayComponentTypescriptSurfaceDebt: report.inventory.reactOverlayComponentTypescriptSurfaceDebt,
    outputs: [
