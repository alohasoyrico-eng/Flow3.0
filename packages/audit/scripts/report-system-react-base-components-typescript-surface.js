...
const jsonOutput = path.join(outputDir, "system-react-base-components-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-base-components-typescript-surface.md");
const checkMode = process.argv.includes("--check");
const reactSourceDir = "packages/react/src";
const components = ["Button", "Input", "Card", "Surface"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-base-components-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React base components TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the first React base component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React base component TypeScript surface debt: ${report.inventory.reactBaseComponentTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "base-components-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited base React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more base React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-base-components-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactBaseComponentTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactBaseComponentTypescriptSurfaceDebt: report.inventory.reactBaseComponentTypescriptSurfaceDebt,
    outputs: [
