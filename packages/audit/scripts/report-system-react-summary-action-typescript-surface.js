...
const jsonOutput = path.join(outputDir, "system-react-summary-action-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-summary-action-typescript-surface.md");
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-summary-action-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React summary/action TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React summary/action component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React summary/action TypeScript surface debt: ${report.inventory.reactSummaryActionTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "summary-action-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited summary/action React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more summary/action React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-summary-action-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactSummaryActionTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactSummaryActionTypescriptSurfaceDebt: report.inventory.reactSummaryActionTypescriptSurfaceDebt,
    outputs: [
