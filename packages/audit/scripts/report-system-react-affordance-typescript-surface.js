...
const jsonOutput = path.join(outputDir, "system-react-affordance-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-affordance-typescript-surface.md");
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-affordance-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React affordance TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React affordance component TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React affordance TypeScript surface debt: ${report.inventory.reactAffordanceTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "affordance-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited affordance React component must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more affordance React component runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-affordance-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactAffordanceTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactAffordanceTypescriptSurfaceDebt: report.inventory.reactAffordanceTypescriptSurfaceDebt,
    outputs: [
