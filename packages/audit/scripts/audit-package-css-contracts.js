...
function checkPackageCssContracts() {
  ...
  for (const match of text.matchAll(/--comp-[\w-]+:\s*(?:var|calc|min|max)\([^;]*--sys-a11y-[\w-]+[^;]*;/g)) add("errors", packageCssFile, lineNumber(text, match.index), "Component accessibility aliases must consume --component-* aliases instead of reaching into sys a11y tokens directly.");
  for (const match of text.matchAll(/--comp-[\w-]+:\s*(?:var|calc|min|max|color-mix)\([^;]*--sys-(?:energy|map|chart)-[\w-]+[^;]*;/g)) add("errors", packageCssFile, lineNumber(text, match.index), "Component domain color aliases must consume --component-* aliases instead of reaching into sys domain tokens directly.");
