function checkReactComponentContentGuards({ add, name, sourceFile, source }) {
  ...
  if (name === "Menu" && /"aria-label":\s*menuAriaLabel\s*\|\|\s*label\s*\|\|\s*undefined/.test(source)) add("errors", sourceFile, 1, "Menu panel must not ignore trigger names when resolving its accessible name.");
  ...
  if (name === "Table" && !source.includes("const resolvedColumns = useMemo(() => columns.filter((column) => column?.key && column?.label), [columns]);")) add("errors", sourceFile, 1, "Table must filter columns without visible labels before rendering headers and cells.");
  ...
  if (name === "List" && !source.includes("const hasStableKey = item.key !== undefined && item.key !== null && item.key !== \"\";")) add("errors", sourceFile, 1, "List must require stable item keys before composing interactive rows.");
  if (name === "List" && !source.includes("const itemCanInteract = isInteractive && hasStableKey && Boolean(item.label || item.meta || item.value);")) add("errors", sourceFile, 1, "List must gate interactive rows on stable keys and visible item content.");
