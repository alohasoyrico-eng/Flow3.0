...
const jsonOutput = path.join(outputDir, "system-react-data-selection-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-data-selection-typescript-surface.md");
const checkMode = process.argv.includes("--check");
const reactSourceDir = "packages/react/src";
const components = ["Select", "Combobox", "Table"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-data-selection-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React data selection TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React data and selection control TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React data selection TypeScript surface debt: ${report.inventory.reactDataSelectionTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "data-selection-controls-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited data selection React control must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more data selection React control runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-data-selection-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactDataSelectionTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactDataSelectionTypescriptSurfaceDebt: report.inventory.reactDataSelectionTypescriptSurfaceDebt,
    outputs: [
