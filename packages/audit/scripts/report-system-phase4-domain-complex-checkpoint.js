...
const jsonOutput = path.join(outputDir, "system-phase4-domain-complex-checkpoint.json");
const markdownOutput = path.join(outputDir, "system-phase4-domain-complex-checkpoint.md");

const domainComplexIds = [
  "animated-moment",
  "audit-event",
  "avatar",
  "badge",
...
function createReport() {
  ...
  const components = domainComplexIds.map((id) => {
  ...
  const inventory = {
    domainComplexComponents: domainComplexIds.length,
    passingDomainComplexComponents: components.filter((component) => component.status === "pass").length,
    ...
    domainComplexDebt: issues.length,
  };

  return {
    status: issues.length ? "fail" : "pass",
    audit: "system phase 4 domain/complex checkpoint",
    principle: "Domain, complex, feedback, chat, motion, and display components must pass TypeScript source/runtime/declaration checks plus React primary, props, controlled state, accessibility, interaction, visual cascade, CSS contract, and style governance before Phase 4 can move into final component QA.",
    scope: "Original plan iteration 23: component fixes batch 3, domain/complex components.",
    inventory,
...
function toMarkdown(report) {
  ...
  return [
    ...
    "## Inventory",
    "",
    `- Domain/complex components: ${report.inventory.passingDomainComplexComponents}/${report.inventory.domainComplexComponents}`,
    `- Gate reports: ${report.inventory.passingGateReports}/${report.inventory.gateReports}`,
    `- TSX sources: ${report.inventory.tsxSources}/${report.inventory.domainComplexComponents}`,
    `- Runtime files: ${report.inventory.runtimeFiles}/${report.inventory.domainComplexComponents}`,
    `- Declaration files: ${report.inventory.declarationFiles}/${report.inventory.domainComplexComponents}`,
    `- Component gate edges: ${report.inventory.passingComponentGateEdges}/${report.inventory.componentGateEdges}`,
    `- Domain/complex debt: ${report.inventory.domainComplexDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  if (checkMode) {
    ...
    if (currentJson !== nextJson || currentMarkdown !== nextMarkdown) {
      console.error("Phase 4 domain/complex checkpoint is stale. Run: node packages/audit/scripts/report-system-phase4-domain-complex-checkpoint.js");
      process.exit(1);
    }
  } else {
    writeReport(report);
  ...
  console.log(JSON.stringify({
    status: report.status,
    components: `${report.inventory.passingDomainComplexComponents}/${report.inventory.domainComplexComponents}`,
    ...
    debt: report.inventory.domainComplexDebt,
    json: rel(jsonOutput),
