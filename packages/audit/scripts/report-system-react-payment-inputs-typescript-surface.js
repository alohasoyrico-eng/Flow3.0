...
const jsonOutput = path.join(outputDir, "system-react-payment-inputs-typescript-surface.json");
const markdownOutput = path.join(outputDir, "system-react-payment-inputs-typescript-surface.md");
const checkMode = process.argv.includes("--check");
const reactSourceDir = "packages/react/src";
const components = ["CardNumberInput", "CardExpiryInput", "CardSecurityCodeInput"];
...
function compileExpectedRuntimes() {
  const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "flow-react-payment-inputs-ts-audit-"));
...
function renderMarkdown(report) {
  const gateRows = report.gates
    ...
    .join("\n");
  return [
    "# System React payment inputs TypeScript surface",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "This report governs the React payment input TSX batch. TypeScript is the maintained source, JavaScript is generated runtime, and declaration files remain compatible during incremental migration.",
    "",
    "## Summary",
    "",
    ...
    `- React payment inputs TypeScript surface debt: ${report.inventory.reactPaymentInputsTypescriptSurfaceDebt}`,
    "",
    "## Components",
    "",
...
function main() {
  ...
  const gates = [
    gate(
      "payment-inputs-have-tsx-source-runtime-and-declarations",
      ...
      "Each audited payment React input must have TSX source, JS runtime, and declaration file.",
    ),
    gate(
    ...
    gate(
      ...
      "One or more payment React input runtimes are stale. Run npm run build:react.",
    ),
  ];
  const failing = gates.filter((item) => item.status !== "PASS");
  const report = {
    schemaVersion: "flow-system-react-payment-inputs-typescript-surface@1",
    ...
    inventory: {
      ...
      staleRuntimeFiles,
      reactPaymentInputsTypescriptSurfaceDebt: failing.length + staleRuntimeFiles,
    },
  };
  ...
  console.log(JSON.stringify({
    ...
    reactPaymentInputsTypescriptSurfaceDebt: report.inventory.reactPaymentInputsTypescriptSurfaceDebt,
    outputs: [
