...
Status: complete in the working tree after contract regression correction. `code-input` is closed for current review in `dffa7131` with governed frame geometry, mono/light numerals, visible success momentum, and error shake feedback. `phone-input` is closed for current review in `eee430dc` with governed CountrySelector composition, local Input search, keyboard/value behavior proof, dark-mode and density/frame validation. `input-amount` is closed for current review in the working tree with ZIP InputAmount reference, shared Field frame/radius/density cascade, mono right-aligned numerals, validation semantics, loading/disabled precedence, and a package-backed React runtime demo. `card-number-input` is closed for current review in the working tree with ZIP Input plus PaymentCard reference alignment, grouped mono numerals, local brand/Luhn states, and a package-backed React runtime demo. `card-expiry-input` is closed for current review in the working tree with ZIP InputDate plus PaymentCard reference alignment, mono MM/YY numerals, distinct invalid/expired states, and a package-backed React runtime demo. `card-security-code-input` is closed for current review in the working tree with ZIP InputPassword plus PaymentCard reference alignment, CVV/CVC length validation, optional shared Field Action reveal, and package-backed React runtime demo. `date-picker` and `date-range-picker` were revalidated after removing forced-open fixtures from the interactive demos and binding their field shell gap to the shared focus-ring clearance contract; both now prove initial closed state, Enter open, ArrowDown traversal, Escape close with focus return, measured label/helper clearance while focused, no panel overflow, and clean console in package-backed runtime demos. The Field CSS contract now also detects later root-level `gap` overrides in compound field shells, so cascade order cannot silently defeat the shared clearance token. P0 Forms Batch 3 is closed; the active cursor moved through overlays and now points to `accordion`.
...
Status: complete in the working tree after overlay visual-parity regression correction. `menu` has reviewed runtime demo from the prior pass. `dialog` is closed for current review with ZIP compact panel anatomy measured in runtime: shared compact overlay inline alias, 440px panel width, 28px padding, 24px radius, 18px panel rhythm, 14px header gap, 10px footer gap, initial closed state, Enter/focus/Escape/outside/action proof, and passing CSS/core contracts. `drawer` is closed for current review with package-backed runtime demo support, governed `children` composition restored in React, Overlays/Fleet Dashboard ZIP behavior represented first (`Abrir drawer`, `Ana Sosa`, `En ruta`, driver copy, flat Accordion sections for `Documentos`/`Historial`/`Métodos de pago`, `Cerrar`/`Guardar`), ZIP compact sheet anatomy measured in runtime: 400px md panel width, `component-radius-sheet` consumption, flex-column sheet layout, slot-owned header/body/footer padding, 12px header gap, 10px footer gap, Enter/focus/Escape/outside/side-placement proof, nested Accordion flat-surface proof, and passing CSS/core contracts. `popover` is closed for current review only through documented Tooltip/Menu/Dialog ZIP triangulation, not direct 1:1 homonym parity. The active cursor is `accordion`.
